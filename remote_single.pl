#test script 
for($num_files = 4; $num_files <= 10; $num_files+=2) {
  ...
  for($run = 1; $run <= 3; $run++) {
    ...
    `cp ~/projects/mediakhan/media/1.mp3 ~/projects/mediakhan/remote2`;
    for($file = 2; $file <= $num_files; $file++) {  
      `cp ~/projects/mediakhan/media/$file.mp3 ~/projects/mediakhan/khan_root`;
    }
